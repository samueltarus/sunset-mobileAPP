...
class CartsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!is_null($productVariation)) {
            ...
            removeCoupon();
            return response()->json([
                $this->getCartsInfo($message, false)], 200);
        }
    }
        ...
        if ($coupon) {
            ...
            if ($coupon->start_date <= $date && $coupon->end_date >= $date) {
                ...
                if ($subTotal >= (float) $coupon->min_spend) {
                    ...
                    setCoupon($coupon);
                    return response()->json([
                        $this->getCartsInfo(localize('Coupon applied successfully'), true, $coupon->code)], 200);
                }

                # min spend
                removeCoupon();
                return response()->json([
                    $this->couponApplyFailed('Please shop for atleast ' . formatPrice($coupon->min_spend))], 200);
            }

            # expired 
            removeCoupon();
            return response()->json([
                $this->couponApplyFailed(localize('Coupon is expired'))], 200);
        }

        // coupon not found
        removeCoupon();
        return response()->json([
            $this->couponApplyFailed(localize('Coupon is not valid'))], 200);
    }
    ...
    public function clearCoupon()
    {
        removeCoupon();
        return response()->json([
            $this->couponApplyFailed(localize('Coupon has been removed'), true)], 200);
    }
